...
function ScaleCalculator() {
  ...
  useEffect(() => { 
    if (mode === "SCALE_IN") { if (emiten === "DCBA") { setEmiten("ABCD"); setTotalInput(50000000); } } else { if (emiten === "ABCD") { setEmiten("DCBA"); setTotalInput(1000); } } 
  }, [mode]);
  ...
  const calculateStrategy = () => { 
    ...
    let tempResults = []; let priceStep = (targetPrice - startPrice) / (steps - 1); 
    let accumLot = 0; let accumValue = 0; let totalExecValue = 0; let totalExecLot = 0; 
    for (let i = 0; i < steps; i++) { 
        let rawPrice = startPrice + (i * priceStep); let executedPrice = roundToTick(rawPrice); let weightPct = weights[i] / totalWeight; let currentLot = 0; let currentValue = 0; 
        if (mode === "SCALE_IN") { let allocationRp = totalInput * weightPct; currentLot = Math.floor(allocationRp / (executedPrice * 100)); currentValue = currentLot * 100 * executedPrice; } else { let allocationLot = totalInput * weightPct; currentLot = Math.round(allocationLot); currentValue = currentLot * 100 * executedPrice; } 
        accumLot += currentLot; accumValue += currentValue; totalExecValue += currentValue; totalExecLot += currentLot; let avgPrice = accumValue / (accumLot * 100); 
        tempResults.push({ level: i + 1, price: executedPrice, weightPct: (weightPct * 100).toFixed(1), lot: currentLot, value: currentValue, avgPrice: Math.round(avgPrice || 0) }); 
    } 
    setResults(tempResults); setSummary({ totalMoney: totalExecValue, totalLot: totalExecLot, finalAvg: accumValue / (accumLot * 100) || 0 }); 
  };
  useEffect(() => { calculateStrategy(); }, [mode, totalInput, startPrice, targetPrice, method, multiplier, emiten, isAutoLevel, manualSteps]);

  return (
    <div className="space-y-4">
        <div ref={printRef} className="bg-white rounded-xl shadow-lg overflow-hidden border border-slate-200 relative">
        <Watermark />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 relative z-10">
            ...
            <div className="lg:col-span-2 p-4 bg-white/50 backdrop-blur-sm relative">
            {isInvalidTicker ? (
...
// ==========================================
// 2. KOMPONEN RIGHT ISSUE STRATEGI
// ==========================================
function RightIssueStrategy() {
  ...
  useEffect(() => { 
    ...
    let totalWaran = 0; if (hasWaran) totalWaran = Math.floor((hakTebus * waranNew) / waranOld); 
    const valSisaLama = sisaLotLama * 100 * hargaPasar; const valBaru = danaTebus; 
    const avgPrice = totalLotBaru > 0 ? Math.round((valSisaLama + valBaru) / (totalLotBaru * 100)) : 0; 
    setResult({ recommendation, isWorthy, lotJual, danaMasuk, hakTebus, danaTebus, selisihCash, totalLotBaru, growth, totalWaran, avgPrice }); 
...
function RiTerpCalculator() {
  ...
  const [hargaPasar, setHargaPasar] = useState(400); 
  const [hargaTebus, setHargaTebus] = useState(250); 
  const [ratioOld, setRatioOld] = useState(500); 
  ...
  useEffect(() => { 
    ...
    const totalValOld = hargaPasar * ratioOld; const totalValNew = hargaTebus * ratioNew; const totalShares = ratioOld + ratioNew; 
    const terp = (totalValOld + totalValNew) / totalShares; const dilution = ((terp - hargaPasar) / hargaPasar) * 100;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 space-y-3 relative z-10">
            ...
            <div className="bg-white p-3 border rounded-lg shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="flex-1"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Jml Saham Lama (Juta)</label><input type="text" value={formatNum(ratioOld)} onChange={handleInput(setRatioOld)} className="w-full p-2 border rounded text-center font-bold bg-slate-50" placeholder="500" /><p className="text-[10px] text-slate-400 mt-1 font-mono text-center">= {formatNum(ratioOld * 1000000)} Lbr</p></div>
                    <span className="font-bold text-slate-400 text-lg mt-4">+</span>
                    <div className="flex-1"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Jml Saham Baru (Juta)</label><input type="text" value={formatNum(ratioNew)} onChange={handleInput(setRatioNew)} className="w-full p-2 border rounded text-center font-bold bg-slate-50" placeholder="100" /><p className="text-[10px] text-slate-400 mt-1 font-mono text-center">= {formatNum(ratioNew * 1000000)} Lbr</p></div>
                </div>
        ...
        <div className="p-6 bg-white/50 backdrop-blur-sm relative z-10">
            {result && (
                <div className="space-y-4">
                <div className="bg-white p-4 rounded-xl border border-orange-200 shadow-sm relative overflow-hidden text-center">
                ...
                <div className="text-center"><button onClick={() => setShowFormula(!showFormula)} className="text-xs font-bold text-slate-400 underline hover:text-orange-600 transition-colors">{showFormula ? "Sembunyikan Rumus" : "Lihat Cara Hitung"}</button></div>
                {showFormula && (
